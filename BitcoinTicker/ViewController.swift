...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate{
    ...
    override func viewDidLoad() {
        ...
        getPriceData(url: baseURL,row: 0)
    }
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        getPriceData(url: baseURL,row: row)
    }
    ...
    func getPriceData(url: String, row: Int) {
        Alamofire.request(url+currencyArray[row])
    ...
    func updatePriceData(json : JSON,row: Int) {
        ...
        if let priceResult = json["last"].double {
            bitcoinPriceLabel.text = "\(currencySymbol[row])\(priceResult)"
        }
        else {
